...
class Ui_MainWindow(object):
    ...
    def set_signal(self, filenames, total_time, speed, lambda_source, source_bandwith, delta_n, ADC_frequency,
                   phase_modulation_frequency):
        args = [total_time, speed, lambda_source, source_bandwith, delta_n, ADC_frequency, phase_modulation_frequency]
        float_parameters = self.str2float(*args)
    ...
    def open_file(self):
        ...
        self.set_signal(self.filenames, self.total_time.text(), self.speed.text(), self.lambda_source.text(),
                        self.source_bandwith.text(), self.delta_n.text(), self.ADC_frequency.text(),
                        self.phase_modulation_frequency.text())
        self.draw_graph()

    def draw_graph(self):
        visibility = [i.get_visibility() for i in self.signal]
        for plt in [self.graph_widget, self.graph_widget_3]:
            plt.addLegend()
            plt.setLogMode(False, True)
            plt.setLabel('bottom', 'Длина плеча интерферометра', 'м')
            plt.setLabel('left', 'Видность', '')
        for x, y, name in visibility:
            pen = (randrange(255), randrange(255), randrange(255))
            for plt in [self.graph_widget, self.graph_widget_3]:
        ...
        def updatePlot():
            self.graph_widget_3.setXRange(*lr.getRegion(), padding=0)

        def updateRegion():
            lr.setRegion(self.graph_widget_3.getViewBox().viewRange()[0])

        lr.sigRegionChanged.connect(updatePlot)
        self.graph_widget_3.sigXRangeChanged.connect(updateRegion)
        updatePlot()
